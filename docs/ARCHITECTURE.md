...
Knowledge lives where it belongs. Manifests stay on publishers' domains. Discovery runs locally. Each model does what it's best at — Claude for intent classification, conversation, and tool calling; a small local LLM for fingerprinting, email classification, and structured extraction. No central service to go down, get acquired, or start charging rent.
...
- `auth: "bot"` — key as `Authorization: Bot` header
...
When `tool_bridge.use_cloud_tools: true` and escalation is configured, ALL tool calls route through Claude instead of the small local LLM. Discovery still runs locally — manifest selection is done by the small model — but Claude handles the actual tool_use/tool_result loop.

This solves the small LLM's inconsistent tool calling without sacrificing the privacy and cost benefits of local discovery. The cloud tool bridge converts Ollama-format tool definitions to Claude's `input_schema` format and manages the multi-round tool execution loop.

Config: `tool_bridge.use_cloud_tools: true` (requires `escalation:` block with provider + model). Response metadata includes `oap_cloud_tools: true` when tools were actually invoked through Claude.

Key file: `cloud_tool_bridge.py`.

### Debug Mode

`POST /v1/chat` accepts `oap_debug: true` for full execution trace: tools discovered, experience cache status, fingerprint, hints, thinking/escalation/cloud-tools flags, and per-round tool executions with timing.
...
- **Synthetic memory** — two-layer system: semantic facts (short durable facts, 3–15 words, UNIQUE dedup, LRU eviction at 2000 max) and episodic memories (1–3 sentence narratives with emotional valence, intensity, people, location, tags). Weighted retrieval: `similarity*0.4 + strength*0.3 + intensity*0.2 + recency*0.1`. See [SYNTHETIC-MEMORY.md](SYNTHETIC-MEMORY.md) for the full design.
- **Visual memory** — images pasted or uploaded into chat are saved to disk, analyzed by the vision model, and linked to extracted facts/episodes. When relevant memories surface, the saved image is included in the response as a markdown image.
- **Intent classification** — 3-tier: (1) fast regex for obvious cases, (2) big LLM classifier (Claude, ~$0.001) for ambiguous messages, (3) fallback to tool discovery. When escalation is enabled, all conversational messages route to Claude for higher quality.
- **Voice** — local STT via faster-whisper (CTranslate2) + TTS via Piper neural voices, both running on the backend. Wake word detection ("Computer") with continuous listening.
...
**Place-based reminders** — the `place` field enables location-triggered reminders without due dates. "Next time I go to the store, remind me to buy milk" creates a reminder with `place: "store"`. When the user mentions a location, the reminder surfaces. The `place` action queries by location with case-insensitive substring matching and article stripping.

**Date range queries** — `due` action supports `after`/`before` parameters for "this week", "next month" queries. Safety guardrail: the system prompt instructs the LLM to never complete or delete reminders without explicit user request.
...
Combined category + priority classification in a single JSON-mode LLM call. Categories: `personal`, `machine`, `mailing-list`, `spam`, `offers`. Priorities: `urgent`, `important`, `informational`, `noise`. The morning briefing filters by `priority=urgent,important` to show only what matters. Supports `use_escalation: true` to route classification to Claude for higher accuracy.

**Sender overrides** — two layers checked before LLM classification: (1) DB overrides managed conversationally ("mark emails from @facebook.com as noise") and (2) config YAML overrides. DB takes precedence. Pattern matching supports exact email or @domain.

Auto-filing moves classified messages into IMAP folders based on category. Designed for cron: `curl -s -X POST localhost:8305/scan && curl -s -X POST localhost:8305/file`.
