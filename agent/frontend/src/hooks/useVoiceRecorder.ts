...
export function useVoiceRecorder(onResult: (text: string) => void) {
  ...
  // No separate function needed — recording starts on onset via startMediaRecorder
  ...
  function goPassive() {
    ...
    setListening(true)
    setAttentive(false)
    // No recorder running in passive — it starts on onset detection
  ...
  const start = useCallback(async (opts?: StartOptions) => {
    ...
    try {
      ...
      if (continuous) {
        ...
        // No recorder in passive — starts on onset detection
        ...
        function updateLevelContinuous() {
          ...
          if (state === 'passive') {
            ...
            // Cap at 0.65 — high enough that TV audio through AGC won't trigger,
            // but direct speech into the mic still will
            const onsetThreshold = Math.min(0.65, Math.max(SPEECH_THRESHOLD, ambientLevelRef.current * 2.5))
