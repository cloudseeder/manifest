...
- **TODO — Manifest hot-reload**: Watch `discovery/manifests/` for file changes (new or updated JSON) and automatically re-index without a full service restart. Use `watchdog` or `asyncio`+`inotify` to detect changes, then re-run the local manifest indexing path from `api.py` lifespan. launchd `KeepAlive` restart is the current workaround but loses the warm Ollama model and ChromaDB state.
